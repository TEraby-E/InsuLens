...
def generate_launch_description() -> LaunchDescription:
    package_share = get_package_share_directory("doggo_perception")
    config = os.path.join(package_share, "config", "detector.yaml")
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "model_path",
                default_value="/root/doggo/models/insulator_defect_yolov10s.pt",
            ),
            DeclareLaunchArgument("device", default_value="cuda:0"),
            DeclareLaunchArgument(
                "image_topic", default_value="/doggo/camera/image_raw"
            ),
            Node(
